...
os.environ['DYNAMODB_TABLE'] = 'TabelaMercado'
...
def setup_data():
    ...
    sample_dynamodb_item = {
        "PK": "LIST#20250528",
        "SK": "ITEM#12345678-1234-5678-9012-123456789012",
        "name": "Comprar leite",
        "status": "todo",
    ...
    return mock_context, sample_event_valid, sample_event_parsed_body, sample_dynamodb_item
...
@pytest.mark.parametrize("event_body, expected_error", [
    ...
    ({"name": "Comprar leite", "date": "2025-05-28"}, "Date must be in YYYYMMDD format."),
    ({"name": "Comprar leite", "date": "2025528"}, "Date must be in YYYYMMDD format."),
...
def test_error_response():
    ...
    response = lambda_handler({"body": json.dumps({"name": "", "date": "20250528"})}, None)
    parsed_response = parse_response(response)
